...
module Wukong
  module Hadoop
    ...
    # This line isn't real yet; it will load from machine level, user level and app level config files in that order
    # Configuration.load_scoped_configuration(:wukong_hadoop) 
    #
    # Hadoop Options
    #
    Configuration.define :hadoop_home, :default => '/usr/lib/hadoop', :description => "Path to hadoop installation; ENV['HADOOP_HOME'] by default. HADOOP_HOME/bin/hadoop is used to run hadoop.", :env_var => 'HADOOP_HOME', :wukong => true
    ...
    Configuration.define :max_record_length,      :jobconf => true,   :description => 'mapred.linerecordreader.maxlength',                      :wukong => true # "Safeguards against corrupted data: lines longer than this (in bytes) are treated as bad records."
    ...
    Configuration.define :noempty,                                    :description => "don't create zero-byte reduce files (hadoop mode only)", :wukong => true
    ...
    def execute_hadoop_workflow
      ...
      #
      # Use Configuration[:hadoop_home] to set the path your config install.
      hadoop_commandline = [
        hadoop_runner,
